...
const candyImages = [
  "https://images.unsplash.com/photo-1610550246952-0c906d3aca7a?w=400",
...
export default function SweetGrid({ sweets, isAdmin, onUpdate }: Props) {
  ...
  const editSweet = async (sweet: Sweet) => {
    ...
    const price =
      Number(prompt("New price", sweet.price.toString())) || sweet.price;
    const quantity =
      Number(prompt("New quantity", sweet.quantity.toString())) ||
      sweet.quantity;
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
      {sweets.map((sweet, i) => (
        ...
        >
          ...
          <div className="p-6">
            <h3 className="text-2xl font-bold">{sweet.name}</h3>
            <p className="text-purple-600">{sweet.category}</p>

            <p className="text-3xl font-bold text-pink-600 my-3">
